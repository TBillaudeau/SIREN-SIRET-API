...
For the database, we have used a dataset with the **siret** number of many companies. We also have their name, their siren number and many others. 

Our project aims to look for a siret of a company. Once we type the siret number we can do several actions such as **get**, **delete** and **add**.

We can do API requests in order to display and retrieve these information.
...
- Create an API able to get, add or delete a siret number
...
Now, the response time is very quick (<2 seconds) !
...
- [Create supertest](https://www.npmjs.com/package/supertest)